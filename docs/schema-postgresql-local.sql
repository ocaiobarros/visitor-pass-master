...
-- RPC: report_vehicle_sessions
CREATE OR REPLACE FUNCTION public.report_vehicle_sessions(
  p_plate text DEFAULT NULL, p_owner text DEFAULT NULL,
  p_start text DEFAULT NULL, p_end text DEFAULT NULL, p_limit int DEFAULT 200, p_offset int DEFAULT 0
) RETURNS TABLE(
  vehicle_plate text, vehicle_model text, person_name text, person_type text,
  document text, gate_id text, entry_time timestamptz, exit_time timestamptz,
  duration_minutes int, session_status text
) LANGUAGE sql SECURITY DEFINER SET search_path TO 'public' AS $$
  WITH evts AS (
...
-- RPC: report_visitors_operational
CREATE OR REPLACE FUNCTION public.report_visitors_operational(
  p_status text DEFAULT NULL, p_start text DEFAULT NULL, p_end text DEFAULT NULL,
  p_limit int DEFAULT 200, p_offset int DEFAULT 0
) RETURNS TABLE(
  id uuid, full_name text, document text, company_name text, company_reason text,
  visit_to_name text, visit_to_type text, access_type text, gate_id text,
  entry_time timestamptz, exit_time timestamptz, duration_minutes int,
  operational_status text, valid_from timestamptz, valid_until timestamptz
) LANGUAGE sql SECURITY DEFINER SET search_path TO 'public' AS $$
